...
(function () {
    ...
    /**
     * Contruct a new deferred out of a function
     *
     * @param next
     *      A function to contruct a sequenced deferred
     *      (or)
     *      A deferred function
     */
    DeferredPrototype.prototype.to = function (next, ctx) {
        return this.pipe(lift(next, ctx)).pipe(function(res) {
            return res && res.pipe ? res.pipe(success) : success(res);
        });
    }

    DeferredPrototype.prototype.toFailure = function (next, ctx) {
        return this.pipeFailure(lift(next, ctx)).pipe(function(res) {
            return res && res.pipe ? res.pipe(success) : success(res);
        });
    }
